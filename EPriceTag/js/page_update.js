...
function updateSubmit(){
	...
	var update_data = '{"goodsOrigin":"'+ $('#goods_origin').val() +'","specifications":"'+ $('#goods_specifications').val() +'","marketPrice":' + parseFloat($('#goods_marketprice').val()).toFixed(2) + ',"shopId":"' + $('#goods_shopid').val() +'","goodsNo":"' + $('#goods_No').val() + '","goodsName":"' + $('#goods_name').html() + '","unit":"' + $('#goods_unit').val() + '","salesPrice":' + parseFloat($('#goods_salesprice').val()).toFixed(2) + ',"propmPrice":' + parseFloat($('#goods_propmprice').val()).toFixed(2) + ',"qrCode":"' + data_qrcode + '","visible":' + visible_radio + ',"id":' + parseInt(data_id) + ',"type":' + parseInt($('#goods_type').val()) + '}';
	// alert(update_data);
...
function updateAllSubmit(){
	...
	var update_goods_no = $('#goods_No').val();
	var update_data = '{"goodsOrigin":"'+ $('#goods_origin').val() +'","specifications":"'+ $('#goods_specifications').val() +'","marketPrice":' + parseFloat($('#goods_marketprice').val()).toFixed(2) + ',"shopId":"' + $('#goods_shopid').val() +'","goodsNo":"' + $('#goods_No').val() + '","goodsName":"' + $('#goods_name').html() + '","unit":"' + $('#goods_unit').val() + '","salesPrice":' + parseFloat($('#goods_salesprice').val()).toFixed(2) + ',"propmPrice":' + parseFloat($('#goods_propmprice').val()).toFixed(2) + ',"qrCode":"' + data_qrcode + '","visible":' + visible_radio + ',"id":' + parseInt(data_id) + ',"type":' + parseInt($('#goods_type').val()) + '}';
	// alert(g_url+'/'+update_goods_no);
	// alert(update_data);
	$.ajax({
		type : 'PATCH',
		url : g_url+'/'+update_goods_no+'/all',
		contentType : 'application/json',
		async : false,
		data : update_data,
		crossDomain : true,
